...
# *现在报装上用不了有点依赖的报错！停更一坤年！现在别用了……我回来改bug再说……
...
- [ ] 修复紧急bug
  - [ ] axios依赖报错与导入模块报错
  - [ ] kimi调用及refreshtoken的自生成
  - [ ] 永劫大神战绩调用
  - [ ] 影画廊的超清图源

### 普通优先级
- [ ] 功能待实现todo
  - [ ] 网易大神全游戏数据推送（所有游戏，不局限于永劫）
  - [ ] 永劫攻略（各魂玉技能信息以及英雄强度排行之类的）
  - [ ] 贴吧乐子（其实这个我有点不想搞，逆天太多了脏眼睛）
  - [ ] 拼多多逆天图片（我不可能写瑟图的，有损功德）
*注,pdd的逆向已经实现一半了，目前的状态是py能正常调用，js的axios一调就报错，不知道为什么……（流汗）*
*详细地址：[拼夕夕逆向研究](https://gitee.com/njmxye/pdd-reverse)*
  - [ ] 永劫无间玩家账号的在线状态（在线，求组，离线……）
  - [ ] 王者荣耀玩家账号的在线状态（在线，离线，排位开局x分钟……）
  - [ ] 王者营地推送与查询
  - [ ] 王者战力榜与自助修改战区
  - [ ] 王者战队信息动态获取
  - [ ] 历史上的今日（每日历史tips）
